...
static void s3_do_cmd_bitblt(S3TrioState *s)
{
    ...
    for (y = 0; y < height; y++) {
        int sy = srcy + y * dy;
        int dy_pos = desty + y * dy;

        if (sy < 0 || sy >= res_height ||
            dy_pos < 0 || dy_pos >= res_height) {
            continue;
        }

        for (x = 0; x < width; x++) {
            int sx = srcx + x * dx;
            int dx_pos = destx + x * dx;
            uint64_t src_off, dest_off;

            if (sx < 0 || sx >= res_width ||
                dx_pos < 0 || dx_pos >= res_width) {
                continue;
            }

            src_off = (uint64_t)sy * res_width + sx;
            dest_off = (uint64_t)dy_pos * res_width + dx_pos;
            if (src_off >= s->vga.vram_size ||
                dest_off >= s->vga.vram_size) {
                continue;
            }

            s->vga.vram_ptr[dest_off] = s->vga.vram_ptr[src_off];
            memory_region_set_dirty(&s->vga.vram, dest_off, 1);
        }
    }
}
...
static const VMStateDescription vmstate_s3_trio = {
    .name = TYPE_S3_TRIO,
    .version_id = 1,
...
static const Property s3_trio_properties[] = {
    DEFINE_PROP_UINT32("vgamem_mb", S3TrioState, vga.vram_size_mb, 4),
};
...

{
    ...
    if (s->vga.vram_size_mb != 1 && s->vga.vram_size_mb != 2 &&
        s->vga.vram_size_mb != 4) {
        error_setg(errp, "S3 Trio vgamem_mb must be 1, 2, or 4");
        return;
    }
    ...
    s->vga.con = qemu_graphic_console_create(DEVICE(s), 0,
                                             s->vga.hw_ops, &s->vga);

    s->vga.get_bpp = s3_trio_get_bpp;

    portio_list_init(&s->portio, o, s3_trio_portio_list, s, "s3_trio");
