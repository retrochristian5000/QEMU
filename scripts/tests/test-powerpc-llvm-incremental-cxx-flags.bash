...
# Persist each output-affecting language profile independently. A changed
# optimization knob must invalidate the installed-toolchain marker once, while
# identical subsequent invocations remain eligible for the fast current exit.
...
# LLD must inherit the exact split profile from the completed base marker.
# Key its own marker to that base marker so changing the C++ optimization knob
# cannot rebuild Clang while accidentally leaving an older LLD untouched.
