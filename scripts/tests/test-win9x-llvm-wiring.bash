...
grep -Fq -- 'shared_llvm="$I386_TOOLCHAIN_DIR/llvm/bin"' "$profile"
grep -Fq -- 'if [[ ! -x "$shared_llvm/lld-link" ]]; then' "$profile"
grep -Fq -- 'if [[ -L "$I386_TOOLCHAIN_DIR/llvm" ]]; then' "$profile"
