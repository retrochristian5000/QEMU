...
TCG. Recovery starts with ``ppc-darwin-user``: 32-bit PowerPC is the oldest
Darwin/macOS application ABI represented by QEMU's historical Darwin-user
implementation and gives the loader a big-endian target immediately. The
PowerPC lane then expands to PPC64/G5 before Intel and AArch64 targets.
...
The implementation order is intentionally historical rather than
host-convenience driven:

``PowerPC32 -> PPC64/G5 -> i386 -> x86_64 -> AArch64 -> arm64e``.

OS X 10.4 introduced the first limited 64-bit user-space support while Apple's
Intel transition added 32-bit Intel support; broader 64-bit framework support
arrived in OS X 10.5. This makes PowerPC the cleanest baseline for recovering
pre-Intel Darwin behavior before later ABIs are layered on top.
...
facts QEMU needs. Start with PowerPC32 and PPC64 thread/Mach-O layouts, then add
Intel and AArch64 layouts. Compile-time size/count tests must make host SDK
layout irrelevant.

**Stage 2 -- modern Mach-O probe/loader**

Start with a byte-oriented parser for classic PowerPC Mach-O. PowerPC makes
endianness impossible to hide: the loader must recognize big-endian 32-bit
headers without casting file bytes onto host C structures. Validate command
bounds before any mapping. Historical ``machload.c`` is a behavioral reference
only.

After the PPC32 ``LC_UNIXTHREAD``/segment path is stable, add PPC64/G5 and
then the modern 64-bit command families needed by Intel and AArch64 fixtures.
...
PowerPC Mach-O fixture. The first executable fixture should be generated or
validated by the pinned WHP LLVM toolchain and must avoid claiming general
...
Expand in historical/ABI layers: PPC32 first, PPC64/G5 second, then i386,
x86_64, ordinary AArch64, and finally arm64e. arm64e requires its own
...
.. list-table::
   ...
   * - macOS AArch64
     - Darwin PowerPC32
     - big-endian Mach-O parse and LC_UNIXTHREAD entry-state fixture
   * - macOS AArch64
     - Darwin PowerPC32
   ...
   * - macOS AArch64
     - Darwin PPC64/G5
     - 64-bit PowerPC parser/ABI fixtures
   * - macOS AArch64
     - Darwin i386 then x86_64
     - Intel ABI expansion after PowerPC
